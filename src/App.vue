...
async function callApi(url, targetVar = 'standard') {
  ...
  try {
    ...
    else if (targetVar === 'grimoire') grimoireResult.value = data; // CIBLE GRIMOIRE
...
// --- MASTERMIND (NOUVEAU) ---
async function runGrimoireScan() { requireVip(async () => {
...
async function runGrimoireConvergence() { requireVip(async () => {
  grimoireResult.value = null;
...
<template>
  ...
  <main v-else class="dashboard">
    <header>
      <h1>LE GUIDE <span class="version-tag">V96.5</span></h1>
      <div class="user-info">
    ...
    <div class="main-layout">
      ...
      <div class="controls-column">
        ...
        <!-- MASTERMIND (NOUVEAU) -->
        <section class="card mastermind-card">
          <div class="boss-header"><h2>🧙‍♂️ LE MASTERMIND</h2><span class="badge-spec">VIP</span></div>
          <div class="tabs">
            <button @click="grimoireMode='scan'" :class="{active: grimoireMode==='scan'}">CIBLE</button>
            <button @click="grimoireMode='converge'" :class="{active: grimoireMode==='converge'}">CONVERGENCE</button>
          </div>
          <div v-if="grimoireMode==='scan'">
            <p class="small-text">Analysez un numéro selon le Grimoire.</p>
            <div class="date-picker-row"><input type="date" v-model="startDate"/><input type="date" v-model="endDate"/></div>
            <input type="number" v-model="grimoireTarget" placeholder="Numéro Cible"/>
            <button @click="runGrimoireScan" :disabled="isLoading||!grimoireTarget" class="magic-btn"><span v-if="!isVip">🔒 </span>DÉCODER</button>
          </div>
          <div v-if="grimoireMode==='converge'">
            <p class="small-text">Entrez le dernier tirage (10 N°).</p>
            <textarea v-model="grimoireInput" placeholder="Ex: 10 20 30..." rows="2" style="width:100%"></textarea>
            <button @click="runGrimoireConvergence" :disabled="isLoading||!grimoireInput" class="magic-btn"><span v-if="!isVip">🔒 </span>CALCULER LA FORCE</button>
          </div>
        </section>

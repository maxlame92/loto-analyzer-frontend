...
const grimoireResult = ref(null); // AJOUT GRIMOIRE
...
async function callApi(url, targetVar = 'standard') {
  ...
  try {
    ...
    else if (targetVar === 'grimoire') grimoireResult.value = data; // AJOUT GRIMOIRE
...
<template>
  ...
  <main v-else class="dashboard">
    <header><h1>LE GUIDE DES FOURCASTER <span class="version-tag">V86.5</span></h1><div class="user-info"><span>{{ user.email }}</span><button @click="logout" class="logout-button">Déconnexion</button></div></header>

    <div class="main-layout">
      <!-- COLONNE GAUCHE (CONTROLS V86) -->
      <div class="controls-column">
        ...
        <!-- IA AVANCEE -->
        <section class="card"><h2>IA Avancée</h2><div style="display:flex; gap:5px; margin-bottom:10px;"><input type="date" v-model="startDate"/><input type="date" v-model="endDate"/></div><input type="text" v-model="triggerInput" placeholder="Cible (ex: 18 ou 12-45)"/><button @click="runTriggerAnalysis" :disabled="isLoading||!triggerInput">Déclencheurs ⚡</button><hr><div class="button-group-horizontal"><button @click="runKantaReport('daily-rank')">Kanta J</button><button @click="runKantaReport('weekly-rank')">Kanta S</button></div></section>

        <!-- NOUVEAU BLOC : MASTERMIND -->
        <section class="card mastermind-card" style="border-top:4px solid #9c27b0; background:#f3e5f5;">
          <div class="boss-header"><h2>🧙‍♂️ LE GRIMOIRE</h2><span class="badge-spec" style="background:#9c27b0;">MAGIC</span></div>
          <div class="tabs">
          ...
          <div v-if="grimoireMode==='converge'">
            <textarea v-model="grimoireInput" placeholder="Ex: 10 20 30..." rows="2" style="width:100%; padding:8px; border-radius:4px; border:1px solid #ccc;"></textarea>
      ...
      <div class="results-column">
        ...
        <div v-if="grimoireResult" class="card result-spec-card" style="border-top:4px solid #9c27b0;">
           ...
           <div v-if="grimoireResult.scouts">
              ...
              <div class="summary-grid">
                 <div class="sum-card"><h5>Éclaireurs (Avant)</h5><ul><li v-for="x in grimoireResult.scouts" :key="x.n">{{ x.n }} ({{x.c}})</li></ul></div>
                 <div class="sum-card"><h5>Garde (Avec)</h5><ul><li v-for="x in grimoireResult.guards" :key="x.n">{{ x.n }} ({{x.c}})</li></ul></div>
                 <div class="sum-card"><h5>Héritiers (Après)</h5><ul><li v-for="x in grimoireResult.heirs" :key="x.n">{{ x.n }} ({{x.c}})</li></ul></div>
              </div>
           </div>
        ...
        <div v-if="matrixResult" class="card result-spec-card" style="border-top:4px solid #ff9800;">
           ...
           <div v-if="matrixResult.prediction" class="prediction-tab"><div class="best-duo-box" style="background: linear-gradient(135deg, #6a11cb 0%, #2575fc 100%);"><span class="duo-label">🔮 PRÉDICTION ({{ matrixResult.prediction.target_date_label }}) :</span><span class="duo-val">{{ matrixResult.prediction.two_short }}</span></div><div class="ai-analysis" style="background:#e8eaf6; color:#1a237e;"><h4>💡 Top Formules :</h4><ul><li v-for="(f, i) in matrixResult.prediction.top_formulas_rich" :key="i"><strong>{{ f.name }}</strong> ({{ f.count }} Hits) - <em>{{ f.best_times }}</em></li></ul></div></div><div class="table-responsive"><table class="spec-table"><thead><tr><th>Date</th><th>Base</th><th>Hits</th></tr></thead><tbody><tr v-for="(row, idx) in matrixResult.matrix_data" :key="idx"><td>{{ row.date }}</td><td class="num-cell">{{ row.base_number }}</td><td><div v-for="h in row.detailed_hits" :key="h.num"><span class="badge-hit">{{ h.num }}</span> ({{ h.time }} - {{ h.reason }})</div></td></tr></tbody></table></div>
        </div>

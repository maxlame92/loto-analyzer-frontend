...
// VARIABLES UNIFIEES MAIS REUTILISÉES LOCALEMENT
...
// INPUTS RESTAURES V80/V82 (Un set de dates par carte)
const rangeStart = ref(''); const rangeEnd = ref('');
const matrixStart = ref(''); const matrixEnd = ref(''); const matrixMode = ref('continuous');
const specStart = ref(''); const specEnd = ref(''); const selectedDayName = ref('Mercredi'); const selectedHour = ref('Toute la journée');
const favStart = ref(''); const favEnd = ref(''); const favDayName = ref('Tous'); const favHour = ref('Toutes');
const visuStart = ref(''); const visuEnd = ref('');
const profileStart = ref(''); const profileEnd = ref(''); const profileNumber = ref('');
const prophetStart = ref(''); const prophetEnd = ref(''); const predictionNumber = ref(''); const predictionCompanion = ref('');
const triggerStart = ref(''); const triggerEnd = ref(''); const triggerInput = ref('');
...
onMounted(() => {
  ...
  // INIT DE TOUTES LES DATES
  selectedDate.value = todayStr;
  rangeStart.value = startStr; rangeEnd.value = todayStr;
  matrixStart.value = startStr; matrixEnd.value = todayStr;
  specStart.value = startStr; specEnd.value = todayStr;
  favStart.value = startStr; favEnd.value = todayStr;
  visuStart.value = startStr; visuEnd.value = todayStr;
  profileStart.value = startStr; profileEnd.value = todayStr;
  prophetStart.value = startStr; prophetEnd.value = todayStr;
  triggerStart.value = startStr; triggerEnd.value = todayStr;
...
const handleAuth = async () => {
  ...
  } catch (error) { authError.value = "Erreur d'authentification."; } 
...
async function runRangeAnalysis() {
  ...
  await callApi(`/analysis/frequency-by-range?start_date=${rangeStart.value}&end_date=${rangeEnd.value}`, 'standard');
}

async function runTimeMatrix() { requireVip(async () => {
  matrixResult.value = null;
  let url = `/analysis/time-matrix?start_date=${matrixStart.value}&end_date=${matrixEnd.value}&mode=${matrixMode.value}`;
...
async function runDayAnalysis() { requireVip(async () => {
  await callApi(`/analysis/specific-day-recurrence?day_name=${selectedDayName.value}&target_hour=${selectedHour.value}&start_date=${specStart.value}&end_date=${specEnd.value}`, 'specialist');
});}

async function analyzeDeepFavorite(item) { requireVip(async () => {
  deepFavoriteResult.value = null;
  await callApi(`/analysis/deep-favorite?target=${item}&start_date=${favStart.value}&end_date=${favEnd.value}`, 'deep');
});}

async function runBatchVisualAnalysis(mode) { requireVip(async () => {
  lastOperationType.value = 'visual'; 
  await callApi(`/analysis/highlight-range?start_date=${visuStart.value}&end_date=${visuEnd.value}&mode=${mode}`, 'standard'); 
});}

// RAPPORTS PONCTUELS (DATE UNIQUE)
...
async function runProfileAnalysis() { requireVip(async () => {
  ...
  await callApi(`/analysis/number-profile?target_number=${profileNumber.value}&start_date=${profileStart.value}&end_date=${profileEnd.value}`, 'profile');
});}

async function runPredictionAnalysis() { requireVip(async () => {
  lastOperationType.value = 'simple';
  let url = `/analysis/predict-next?observed_number=${predictionNumber.value}&start_date=${prophetStart.value}&end_date=${prophetEnd.value}`;
...
async function runTriggerAnalysis() { requireVip(async () => {
  ...
  let url = `/analysis/trigger-numbers?target_number=${parts[0]}&start_date=${triggerStart.value}&end_date=${triggerEnd.value}`; 
  ...
  <main v-else class="dashboard">
    ...
    <div class="main-layout">
      ...
      <div class="controls-column">
        ...
        <!-- SECTION ADMIN (RETOUR) -->
        <section v-if="isAdmin" class="card data-update">
          <h2>MAINTENANCE</h2>
          <div class="button-group-horizontal">
            <button @click="runDataUpdate('update-recent-weeks')" :disabled="isLoading">MISE À JOUR</button>
            <button @click="runDataUpdate('start-full-rebuild')" :disabled="isLoading" class="danger">REBUILD</button>
          </div>
        </section>
        ...
        <section class="card free-card">
          ...
          <div class="date-picker-row"><input type="date" v-model="rangeStart"/><input type="date" v-model="rangeEnd"/></div>
          <button @click="runRangeAnalysis" :disabled="isLoading||!rangeStart" class="free-btn">LANCER L'ANALYSE GRATUITE</button>
        </section>
        ...
        <section class="card matrix-card">
          ...
          <div class="date-picker-row"><input type="date" v-model="matrixStart"/><input type="date" v-model="matrixEnd"/></div>
          <button @click="runTimeMatrix" :disabled="isLoading" class="spec-btn"><span v-if="!isVip">🔒 </span>ANALYSER & PRÉDIRE</button>
        </section>
        
        <!-- SPECIALISTE (Avec Dates) -->
        <section class="card spec-card">
          ...
          <div class="date-picker-row"><input type="date" v-model="specStart"/><input type="date" v-model="specEnd"/></div>
        ...
        <section class="card">
          ...
          <div class="date-picker-row"><input type="date" v-model="favStart"/><input type="date" v-model="favEnd"/></div>
        ...
        <section class="card">
          ...
          <div class="date-picker-row"><input type="date" v-model="visuStart"/><input type="date" v-model="visuEnd"/></div>
          <div class="button-group-vertical">
             <button @click="runBatchVisualAnalysis('frequency')" :disabled="isLoading||!visuStart" class="visu-btn"><span v-if="!isVip">🔒 </span>Rouge/Bleu (Période)</button>
             <button @click="runBatchVisualAnalysis('kanta')" :disabled="isLoading||!visuStart" class="visu-btn"><span v-if="!isVip">🔒 </span>Kanta (Période)</button>
          </div>
        </section>
        ...
        <!-- PROFIL (Avec Dates) -->
        <section class="card"><h2>Période & Profilage</h2>
          <div class="date-picker-row"><input type="date" v-model="profileStart"/><input type="date" v-model="profileEnd"/></div>
          <input type="number" v-model="profileNumber" placeholder="N° Profil"/>
          <button @click="runProfileAnalysis" :disabled="isLoading||!profileStart||!profileNumber"><span v-if="!isVip">🔒 </span>Générer Profil</button>
        </section>
        
        <!-- PROPHETE (Avec Dates) -->
        <section class="card prophet-card"><h2>🔮 Le Prophète</h2>
          <div class="date-picker-row"><input type="date" v-model="prophetStart"/><input type="date" v-model="prophetEnd"/></div>
          <input type="number" v-model="predictionNumber" placeholder="N° vu"/>
          <input type="number" v-model="predictionCompanion" placeholder="Compagnon"/>
          <button @click="runPredictionAnalysis" :disabled="isLoading||!prophetStart||!predictionNumber" class="prophet-btn"><span v-if="!isVip">🔒 </span>Voir Futur</button>
        </section>
        
        <!-- TRIGGER (Avec Dates & Input Cible) -->
        <section class="card"><h2>IA Avancée</h2>
          <div class="date-picker-row"><input type="date" v-model="triggerStart"/><input type="date" v-model="triggerEnd"/></div>
          <input type="text" v-model="triggerInput" placeholder="Cible (ex: 18 ou 12-45)"/>
          <button @click="runTriggerAnalysis" :disabled="isLoading||!triggerInput"><span v-if="!isVip">🔒 </span>Déclencheurs ⚡</button>
        </section>
      </div>

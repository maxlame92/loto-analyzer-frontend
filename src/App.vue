...
// Compteur Local (Bloquage 3 clics)
...
// LOGIQUE DE SÉCURITÉ : 3 ESSAIS PAR FONCTION
...
const chartData = computed(() => {
  ...
  return { labels: data.slice(0, 15).map(r => r.number || r.pair || '?'), datasets: [{ label: 'Hits', backgroundColor: '#4361ee', data: data.slice(0, 15).map(r => r.count || r.total_hits) }] };
});
...
onMounted(async () => {
  ...
  } catch (e) { console.warn("Compteur bridé."); }
...
const signup = async () => {
  if (isLimitReached.value) { authError.value = "Limite d'utilisateurs (50) atteinte."; return; }
  try {
    isLoading.value = true;
...
async function runTimeMatrix() { if(!canUse('matrix')) return; matrixResult.value = null; await callApi(`/analysis/time-matrix?start_date=${startDate.value}&end_date=${endDate.value}&mode=${matrixMode.value}${matrixMode.value==='cyclic'?'&target_cyclic_day='+cyclicDay.value:''}`, 'matrix'); }
async function runDayAnalysis() { if(!canUse('sniper')) return; await callApi(`/analysis/specific-day-recurrence?day_name=${selectedDayName.value}&target_hour=${selectedHour.value}&start_date=${startDate.value}&end_date=${endDate.value}`, 'specialist'); }
async function runReport(type) {
  if(!canUse('report')) return;
...
async function runRangeAnalysis() { if(!canUse('report')) return; lastOperationType.value = 'ranking_rich'; await callApi(`/analysis/frequency-by-range?start_date=${startDate.value}&end_date=${endDate.value}`); }
async function runProfileAnalysis() { if(!canUse('profile')) return; lastOperationType.value = 'profile'; await callApi(`/analysis/number-profile?target_number=${profileNumber.value}&start_date=${startDate.value}&end_date=${endDate.value}`, 'profile'); }
async function runTriggerAnalysis() { if(!canUse('trigger')) return; lastOperationType.value = 'simple'; const p = triggerInput.value.split(' '); await callApi(`/analysis/trigger-numbers?target_number=${p[0]}&start_date=${startDate.value}&end_date=${endDate.value}`); }
async function runPredictionAnalysis() { if(!canUse('prophet')) return; lastOperationType.value = 'simple'; await callApi(`/analysis/predict-next?observed_number=${predictionNumber.value}&start_date=${startDate.value}&end_date=${endDate.value}`); }
async function runKantaReport() { if(!canUse('report')) return; lastOperationType.value = 'simple'; await callApi(`/analysis/kanta-daily-rank?date_str=${selectedDate.value}`); }
async function runBatchVisual(mode) { if(!isAdmin.value) return; await callApi(`/analysis/highlight-range?start_date=${startDate.value}&end_date=${endDate.value}&mode=${mode}`); }
async function runSingleVisual(mode) { if(!isAdmin.value) return; await callApi(`/analysis/highlight-range?start_date=${selectedDate.value}&end_date=${selectedDate.value}&mode=${mode}`); }
async function addFavorite() { if(!canUse('favorite')) return; if(newFavoriteInput.value) { await updateDoc(doc(db,"users",user.value.uid), {favorites: arrayUnion(newFavoriteInput.value)}); userFavorites.value.push(newFavoriteInput.value); newFavoriteInput.value=''; } }
...
async function analyzeDeepFavorite(i) { if(!canUse('favorite')) return; deepFavoriteResult.value = null; await callApi(`/analysis/deep-favorite?target=${i}&start_date=${startDate.value}&end_date=${endDate.value}`, 'deep'); }
</script>
...
<template>
  ...
  <div v-else-if="!user" class="login-wrapper">
    <div class="login-box">
      ...
      <p style="color:#64748b; margin-bottom:15px;">{{ isLoginMode ? 'Connexion Membre' : 'Création de compte (Limitée à 50)' }}</p>
  ...
  <main v-else class="dashboard">
    <header class="prem-header">
      ...
      <div class="vip-zone">
          <div class="u-meta">
            ...
            <span v-else class="u-free">❄️ Formule Gratuite (3 essais/fonc.)</span>
          </div>
    ...
    <div class="contact-banner">
        🔥 Pour activer le <strong>MODE VIP</strong>, contactez l'expert : 
        <span>📞 +225 0749522365</span> ou <span>📞 +225 0102275973</span> - <strong>MR ABOUCHO MAX ELISER</strong>
    </div>
    ...
    <div class="main-layout">
      ...
      <div class="controls-column">
        <section v-if="isAdmin" class="card admin-card">
          ...
          <div class="quick-link-box"><a :href="sheetDirectLink" target="_blank" class="gsheet-btn">📂 GOOGLE SHEETS DB</a></div>
          <hr>
          <div class="date-picker-row"><input type="date" v-model="startDate"/><input type="date" v-model="endDate"/></div>
          <button @click="runBatchVisual('frequency')" style="background:red;">Batch Couleurs Fréq</button>
          <button @click="runBatchVisual('kanta')" style="background:green;">Batch Couleurs Kanta</button>
          <hr>
          <button @click="runSingleVisual('frequency')" style="border:1px solid red; background:transparent; color:red;">🎨 Surligner Jour Unique</button>
        </section>

        <section class="card matrix-card"><div class="card-head"><h2>🕰️ MATRICE</h2><span class="v-badge">VIP</span></div><div class="tabs"><button @click="matrixMode='continuous'" :class="{active: matrixMode==='continuous'}">CONTINU</button><button @click="matrixMode='cyclic'" :class="{active: matrixMode==='cyclic'}">CYCLIQUE</button></div><div class="date-picker-row"><input type="date" v-model="startDate"/><input type="date" v-model="endDate"/></div><button @click="runTimeMatrix" class="act-btn orange">VOIR PRÉDICTIONS</button></section>
        
        <section class="card spec-card"><div class="card-head"><h2>📅 LE SNIPER</h2><span class="v-badge">VIP</span></div><select v-model="selectedDayName"><option v-for="d in ['Lundi','Mardi','Mercredi','Jeudi','Vendredi','Samedi','Dimanche']" :key="d">{{d}}</option></select><button @click="runDayAnalysis" class="act-btn blue">SCANNER JOUR</button></section>
        
        <section class="card"><h2>⭐ MES FAVORIS</h2><div class="fav-box"><input type="text" v-model="newFavoriteInput" placeholder="N°"/><button @click="addFavorite">+</button></div><div class="fav-chips"><div v-for="f in userFavorites" :key="f" class="chip">{{ f }} <span @click="analyzeDeepFavorite(f)">⚡</span><b @click="removeFavorite(f)">×</b></div></div></section>
        
        <section class="card"><h2>📊 RAPPORTS</h2><input type="date" v-model="selectedDate"/><div class="btn-group"><button @click="runReport('daily-frequency')">Top 10 Jour</button><button @click="runReport('weekly-frequency')">Top 10 Semaine</button><button @click="runKantaReport">Kanta Rank</button></div></section>

        <section class="card prophet-card"><h2>🔮 LE PROPHÈTE</h2><input type="number" v-model="predictionNumber" placeholder="N° vu"/><button @click="runPredictionAnalysis" class="proph-btn">VOIR FUTUR</button></section>

        <section class="card"><h2>🔍 PROFILAGE & IA</h2><input type="number" v-model="profileNumber" placeholder="Numéro"/><button @click="runProfileAnalysis">PROFIL COMPLET</button><hr><input type="text" v-model="triggerInput" placeholder="N° ou Paire"/><button @click="runTriggerAnalysis">DÉCLENCHEURS</button></section>
      </div>
      ...
      <div class="results-column">
        ...
        <div v-if="matrixResult" class="card res-card">
          ...
          <div class="scroll-table"><table class="prem-table"><thead><tr><th>Date</th><th>Base</th><th>Hits</th></tr></thead><tbody><tr v-for="r in matrixResult.matrix_data" :key="r.date"><td>{{r.date}}</td><td class="bold">{{r.base_number}}</td><td><span v-for="h in r.detailed_hits" :key="h.num" class="hit-tag">{{h.num}}</span></td></tr></tbody></table></div>
        </div>
        ...
        <div v-if="!matrixResult && !dayAnalysisResult && !standardResult && !isLoading" class="welcome">🎯 Sélectionnez une fonction à gauche pour démarrer.</div>
      </div>
    </div>
  </main>
</template>

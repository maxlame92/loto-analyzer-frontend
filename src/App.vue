...
onMounted(async () => {
  ...
  } catch (e) { console.warn("Erreur permissions compteur."); }
    ...
    if (firebaseUser) {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          userRole.value = 'user';
          userFavorites.value = [];
          await setDoc(docRef, { role: 'user', favorites: [], subscription_end: null }, { merge: true });
        }
      } catch (e) { console.error(e); }
    } else { 
      user.value = null; userRole.value = ''; userFavorites.value = []; subscriptionEnd.value = '';
...
const signup = async () => {
  if (isLimitReached.value) { authError.value = "Désolé, la limite d'utilisateurs est atteinte."; return; }
  try {
    authError.value = ''; isLoading.value = true;
    ...
    await setDoc(doc(db, "users", userCredential.user.uid), {
      email: email.value, role: 'user', favorites: [], subscription_end: null, current_session_id: newSessionId, created_at: new Date().toISOString()
    });
...
async function addFavorite() {
  ...
  if (!input || !/^[0-9]{1,2}(-[0-9]{1,2})?$/.test(input)) { alert("Format invalide."); return; }
...
async function runReport(reportType) {
  ...
  if (reportType === 'daily-frequency') { lastOperationType.value = 'ranking_rich'; await callApi(`/analysis/daily-frequency/${selectedDate.value}`, 'standard'); }
  else if (reportType === 'weekly-frequency') { lastOperationType.value = 'ranking_rich'; await callApi(`/analysis/weekly-frequency/${selectedDate.value}`, 'standard'); }
  else if (reportType === 'companions') { lastOperationType.value = 'simple'; await callApi(`/analysis/companions/${selectedNumber.value}?week_date_str=${selectedDate.value}`, 'standard'); }
}
async function runRangeAnalysis() {
  if (!startDate.value) return;
  lastOperationType.value = 'ranking_rich'; await callApi(`/analysis/frequency-by-range?start_date=${startDate.value}&end_date=${endDate.value}`, 'standard');
}
async function runProfileAnalysis() {
  if (!profileNumber.value) return;
  lastOperationType.value = 'profile'; profileResult.value = null;
...
async function runKantaReport(reportType) { 
  if (!selectedDate.value) { alert("Sélectionnez une date."); return; }
  lastOperationType.value = 'simple'; await callApi(`/analysis/kanta-daily-rank?date_str=${selectedDate.value}`, 'standard'); 
}
</script>

<template>
  <div v-if="!isAuthReady" class="loading-screen"><p>Initialisation...</p></div>
  <div v-else-if="!user" class="login-wrapper">
    <div class="login-box">
      <h2>LE GUIDE DES FOURCASTER</h2>
      <p style="color:#64748b; margin-bottom:20px;">{{ isLoginMode ? 'Connexion' : 'Inscription Gratuite (Limitée à 50)' }}</p>
      <form @submit.prevent="isLoginMode ? login() : signup()">
        ...
        <p class="toggle-auth" @click="isLoginMode = !isLoginMode">{{ isLoginMode ? 'Pas de compte ? S\'inscrire' : 'Déjà un compte ? Se connecter' }}</p>
      </form>
    </div>
  </div>
  ...
  <main v-else class="dashboard">
    <header>
      ...
      <div class="user-info">
        <div class="sub-status">
            ...
            <span v-else class="free-text">❄️ Formule Gratuite</span>
        </div>
    ...
    <div class="vip-banner">
        ...
        <span>📞 +225 0749522365</span> ou <span>📞 +225 0102275973</span>
    </div>
...
header { background: linear-gradient(135deg, #1e293b 0%, #334155 100%); color: white; padding: 1.5rem; border-radius: 12px; margin-bottom: 1.5rem; box-shadow: 0 4px 20px rgba(0,0,0,0.15); display: flex; justify-content: space-between; align-items: center; }
.user-info { display: flex; align-items: center; gap: 20px; }
.sub-status { display: flex; flex-direction: column; text-align: right; }
.expiry-text { font-size: 0.8rem; color: #4cc9f0; font-weight: bold; }
.free-text { font-size: 0.8rem; color: #cbd5e1; }
.vip-banner { background: #fff3e0; border: 1px solid #ff9800; color: #e65100; padding: 12px; border-radius: 10px; text-align: center; margin-bottom: 1.5rem; font-weight: bold; }
.vip-banner span { color: #d32f2f; margin: 0 10px; }
.login-wrapper { display: flex; align-items: center; justify-content: center; height: 100vh; background: #1e293b; }
.login-box { background: white; padding: 2.5rem; border-radius: 25px; width: 400px; text-align: center; box-shadow: 0 20px 50px rgba(0,0,0,0.5); }
.toggle-auth { color: var(--primary); cursor: pointer; margin-top: 15px; font-weight: bold; text-decoration: underline; }
.limit-info { font-size: 0.8rem; color: #ef4444; margin-top: 10px; font-weight: bold; }
...
.card h2 { font-family: 'Roboto', sans-serif; font-weight: 700; text-transform: uppercase; font-size: 1.1rem; color: #333; margin-top: 0; border-bottom: 2px solid rgba(0,0,0,0.05); padding-bottom: 10px; margin-bottom: 15px; }
button { font-family: 'Poppins', sans-serif; font-weight: 600; border: none; border-radius: 10px; padding: 12px; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 4px 6px rgba(0,0,0,0.1); text-transform: uppercase; letter-spacing: 0.5px; font-size: 0.9rem; color: white; background-color: #4361ee; width: 100%; }
...
input, select { padding: 12px; border: 2px solid #e2e8f0; border-radius: 10px; font-family: 'Roboto', sans-serif; font-size: 0.95rem; width: 100%; box-sizing: border-box; margin-bottom: 10px; }
...
.ai-analysis { background: #fff; border-left: 5px solid #f72585; padding: 20px; border-radius: 0 10px 10px 0; font-family: 'Roboto', sans-serif; line-height: 1.6; margin-top: 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.05); }
.ranking-list { display: flex; flex-direction: column; gap: 15px; }
.rank-card { background: white; border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; display: flex; align-items: flex-start; gap: 15px; }
.rank-badge { background: linear-gradient(135deg, #4361ee, #3a0ca3); color: white; border-radius: 50%; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; font-size: 1.2rem; min-width: 40px; }
...
.favorite-chip { background: #e0f2f1; padding: 5px 10px; border-radius: 20px; display: flex; align-items: center; gap: 5px; font-weight: bold; color: #00695c; }
...
.tabs button { flex: 1; padding: 8px; font-size: 0.8rem; background: #673ab7; opacity: 0.6; border-radius: 4px 4px 0 0; }

...
const login = async () => { try { await signInWithEmailAndPassword(auth, email.value, password.value); } catch (e) { authError.value = "Identifiants incorrects"; } };
...
<template>
  ...
  <main v-else class="dashboard">
    <header class="top-nav">
      <h1 class="orbitron logo">FOURCASTER <span class="v-tag">V78.0 PRO</span></h1>
    ...
    <div class="layout">
        ...
        <section class="card">
          ...
          <div class="fav-list">
             <div v-for="f in userFavorites" :key="f" class="f-chip">
                <span>{{f}}</span> <button @click="callApi(`/analysis/deep-favorite?target=${f}&start_date=${startDate}&end_date=${endDate}`, 'deep')">⚡</button>
             </div>
          </div>
        </section>
      ...
      <section class="results-display">
        ...
        <div v-if="isLoading" class="pulse-loader">CALCUL DES ALGORITHMES EN COURS...</div>

        <!-- TABLEAU STRATÉGIQUE (DAILY / WEEKLY / RANGE) -->
        <div v-if="standardResult" class="res-box fade-in">
          ...
          <table class="main-table">
            <thead><tr><th>N°</th><th>SORTIES</th><th>SYNTHÈSE (Top 3)</th></tr></thead>
            <tbody>
        ...
        <!-- PROFIL EXPERT -->
        <div v-if="profileResult" class="res-box profile">
...
.card h3 { font-size: 0.9rem; font-weight: 900; margin-bottom: 10px; border-left: 3px solid #fbbf24; padding-left: 10px; }
...
.profile-stats { display: flex; gap: 20px; justify-content: center; margin-bottom: 25px; }
.p-item { background: #0f172a; padding: 15px; border-radius: 12px; border: 1px solid #fbbf24; width: 120px; text-align: center; }
.p-item p { font-size: 1.3rem; font-weight: 900; margin: 0; color: #fbbf24; }
.pulse-loader { text-align: center; padding: 50px; font-weight: 900; color: #fbbf24; animation: pulse 1.5s infinite; }

...
onMounted(async () => {
  ...
  } catch (e) { console.warn("Permissions compteur."); }
...
const login = async () => {
  try {
    ...
    // Utiliser setDoc avec merge pour être sûr que ça passe même si le doc n'existe pas
    ...
    console.error(err);
    authError.value = "Erreur : " + (err.message.includes("auth/") ? "Identifiants invalides" : "Permission Database refusée"); 
...
const signup = async () => {
  ...
  } catch (err) { authError.value = "Erreur inscription."; } finally { isLoading.value = false; }
};
...
async function runTrigger() { if(!checkLimit('trigger')) return; lastOperationType.value = 'simple'; const parts = triggerInput.value.split(' '); await callApi(`/analysis/trigger-numbers?target_number=${parts[0]}&start_date=${startDate.value}&end_date=${endDate.value}`, 'standard'); }
...
async function addFavorite() { if(!checkLimit('favorite')) return; if (newFavoriteInput.value) { await updateDoc(doc(db, "users", user.value.uid), { favorites: arrayUnion(newFavoriteInput.value) }); userFavorites.value.push(newFavoriteInput.value); newFavoriteInput.value = ''; } }
async function removeFavorite(item) { await updateDoc(doc(db, "users", user.value.uid), { favorites: arrayRemove(item) }); userFavorites.value = userFavorites.value.filter(n => n !== item); }
async function analyzeDeepFavorite(item) { if(!checkLimit('favorite')) return; deepFavoriteResult.value = null; await callApi(`/analysis/deep-favorite?target=${item}&start_date=${startDate.value}&end_date=${endDate.value}&context_day=${favDayName.value}&context_hour=${favHour.value}`, 'deep'); }

// ADMIN
async function runDataUpdate(endpoint) { if(!isAdmin.value) return; await callApi(`/collection/${endpoint}`, 'standard'); }
async function runBatchVisual(mode) { if(!isAdmin.value) return; await callApi(`/analysis/highlight-range?start_date=${startDate.value}&end_date=${endDate.value}&mode=${mode}`, 'standard'); }
...
<template>
  <div v-if="!isAuthReady" class="loading-screen"><p>Chargement Premium V100 PRO...</p></div>
    ...
    <div class="main-layout">
      <div class="controls-column">
        ...
        <section v-if="isAdmin" class="card admin-card">
          ...
          <div class="button-group-horizontal"><button @click="runDataUpdate('update-recent-weeks')">Mise à Jour Site</button><button @click="runDataUpdate('start-full-rebuild')" class="danger">Rebuild</button></div>
          <hr>
          ...
          <button @click="runBatchVisual('frequency')" style="background:#ef5350;">Batch Fréquence</button>
          <button @click="runBatchVisual('kanta')" style="background:#66bb6a;">Batch Kanta</button>
          <hr>
        ...
        <section class="card spec-card">
          ...
          <select v-model="selectedDayName" class="day-select"><option v-for="d in ['Lundi','Mardi','Mercredi','Jeudi','Vendredi','Samedi','Dimanche']" :key="d">{{d}}</option></select>
          <select v-model="selectedHour" class="day-select"><option>Toute la journée</option><option v-for="h in ['01H','03H','07H','08H','10H','13H','16H','19H','21H','22H','23H']" :key="h">{{h}}</option></select>
        ...
        <section class="card">
          <h2>🔍 PROFILAGE</h2>
          <input type="number" v-model="profileNumber" placeholder="N°"/><button @click="runProfile">GÉNÉRER PROFIL</button>
        </section>

        <section class="card prophet-card">
          <h2>🔮 LE PROPHÈTE</h2>
          <input type="number" v-model="predictionNumber" placeholder="N° vu"/><button @click="runPrediction" class="prophet-btn">VOIR LE FUTUR</button>
        </section>
        
        <section class="card"><h2>⚡ DÉCLENCHEURS</h2><input type="text" v-model="triggerInput" placeholder="N° cible"/><button @click="runTrigger">ANALYSER</button></section>
      </div>

      <div class="results-column">
        <div v-if="isLoading" class="loader">Calculs en cours...</div>
        ...
        <div v-if="matrixResult" class="card result-spec-card" style="border-top:4px solid #ff9800;">
           <div class="spec-header"><h3>🕰️ RÉSULTATS MATRICE</h3><button @click="matrixResult=null">×</button></div>
           <div v-if="matrixResult.prediction" class="prediction-tab">
               <div class="best-duo-box" style="background: linear-gradient(135deg, #6a11cb 0%, #2575fc 100%);"><span>🔮 PRÉDICTION :</span><span class="duo-val">{{ matrixResult.prediction.two_short }}</span></div>
           </div>
        ...
        <div v-if="dayAnalysisResult" class="card result-spec-card">
          ...
          <table class="spec-table"><thead><tr><th>N°</th><th>Kanta</th><th>Compagnons</th><th>Déclencheurs</th><th>Prophète</th></tr></thead><tbody><tr v-for="row in dayAnalysisResult.recurrence_data" :key="row.number"><td class="num-cell">{{ row.number }}</td><td style="color:#d32f2f;">{{ row.kanta }}</td><td>{{ row.best_companion }}</td><td>{{ row.best_trigger }}</td><td>{{ row.best_prophet }}</td></tr></tbody></table>
        </div>
        ...
        <div v-if="!dayAnalysisResult && !matrixResult && !standardResult && !isLoading" class="welcome-message">🎯 Sélectionnez une fonction à gauche.</div>
      </div>
    </div>
  </main>
</template>
...
header { background: linear-gradient(135deg, #1e293b 0%, #334155 100%); color: white; padding: 1.5rem; border-radius: 12px; margin-bottom: 1.5rem; box-shadow: 0 10px 20px rgba(0,0,0,0.2); display: flex; justify-content: space-between; align-items: center; }
.version-tag { background: #f72585; padding: 4px 10px; border-radius: 20px; font-size: 0.7rem; }
.sub-status { display: flex; flex-direction: column; text-align: right; margin-right: 15px; }
.expiry-text { font-size: 0.8rem; color: #4cc9f0; font-weight: bold; }
.free-text { font-size: 0.8rem; color: #cbd5e1; }
.contact-banner { background: #fff3e0; color: #e65100; padding: 12px; text-align: center; border-radius: 10px; margin-bottom: 20px; font-weight: bold; }
